...
class UIPS(object):
  def __init__(self, user_count, item_count, user_features, para,  random_seed = 1234,hidden_units=128):
    ...
    self.u_emb = self.getUEmb(self.u, self.user_emb_w, self.user_features, self.user_emb_layers,
                              self.user_features_final_layer)  # [B,H]
    self.i_emb, self.i_b = self.getIemb(self.item_emb_w,self.item_b)  # [B,H], [B,]
  ...
  def getUEmb(self,id,id_emb_layer,features,features_emb_layer,final_layer):
  ...
  def build_beta_network(self):
    ...
    self.beta_i_emb, self.beta_i_b = self.getIemb(self.beta_item_emb_w,  self.beta_item_b)
  ...
  def getIPS(self,displayed_index):
      ...
      phi_sa_1 = self.lambda_ / (self.lambda_ / self.eta_ * diag_beta_dis_uty + self.eta_ * pi_beta_U + 0.00001)
      ...
      tf.summary.scalar('phi_sa/ori_phi_sa_1_mean', tf.reduce_mean(phi_sa_1))
      tf.summary.scalar('phi_sa/ori_phi_sa_1_min', tf.reduce_min(phi_sa_1))
      tf.summary.scalar('phi_sa/ori_phi_sa_1_max', tf.reduce_max(phi_sa_1))
      tf.summary.scalar('phi_sa/ori_phi_sa_2_mean', tf.reduce_mean(phi_sa_2))
      tf.summary.scalar('phi_sa/ori_phi_sa_2_min', tf.reduce_min(phi_sa_2))
      tf.summary.scalar('phi_sa/ori_phi_sa_2_max', tf.reduce_max(phi_sa_2))
      ...
      tf.summary.scalar('phi_sa/mean_part_1: ',  tf.reduce_mean(self.lambda_/self.eta_ * diag_beta_dis_uty))
      tf.summary.scalar('phi_sa/min_part_1: ',  tf.reduce_min(self.lambda_/self.eta_ * diag_beta_dis_uty))
      tf.summary.scalar('phi_sa/max_part_1: ',  tf.reduce_max(self.lambda_/self.eta_ * diag_beta_dis_uty))
      tf.summary.scalar('phi_sa/mean_part_2: ',  tf.reduce_mean( pi_beta_U))
      tf.summary.scalar('phi_sa/min_part_2: ',  tf.reduce_min(pi_beta_U))
      tf.summary.scalar('phi_sa/max_part_2: ',  tf.reduce_max( pi_beta_U))
  ...
  def getIemb(self,item_emb_w,item_b):
