...
parser.add_argument("--topN", default='[5,20]')
...
data_dict = loadData(args.usedata,args.sample_size)
...
#             break
        ...
        # cal avg uncertainty of (x,a) pair
        count_u = 1#0
#         for _, uij in DataInputSyn(train_set, train_batch_size):
#             count_u += len(uij[0])
#             uncertainty = model.getSUncertainty(sess,uij)
#             uncertainty_sum += sum(uncertainty)
                    
        print('Epoch {} Global_step {}\t''[joint]Train_loss: {}\taverage_uncertainty:{}\t'.format(
            model.global_epoch_step.eval(), model.global_step.eval(),
            loss_sum / count, uncertainty_sum/count_u))
        sys.stdout.flush()
